      ...
      case 0:
        for(int j=0; j< dx.size(); j++){
          if(loads[i].getCoordX0() == dx(j)){
            F(2*j) -= loads[i].getLoadValue();  
          } 
        }
        break;
      case 1:
        for(int j=1; j< dx.size(); j++){
          if(dx(j-1) >= loads[i].getCoordX0() && dx(j-1) < loads[i].getCoordX1()){
            F(2*j-2) -= loads[i].getLoadValue() * (dx(j) - dx(j-1))/2;  
            F(2*j-1) -= loads[i].getLoadValue() * pow((dx(j) - dx(j-1)),2)/12;
            F(2*j) -= loads[i].getLoadValue() * (dx(j) - dx(j-1))/2;  
            F(2*j+1) += loads[i].getLoadValue() * pow((dx(j) - dx(j-1)),2)/12;   
          } 
        }
        break;
  ...
  */
  ...
  FILE* pipe = _popen("C:/\"Program Files\"/gnuplot/bin/gnuplot.exe", "w");
    if (pipe != NULL)
    {
        ...
        fprintf(pipe, "set style line 1 lt 1 lw 1 pt 0 linecolor rgb \"red\"\n"); 
        fprintf(pipe, "plot \"./results/results.txt\" with linespoints linestyle 1\n");
